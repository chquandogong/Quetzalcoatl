...
여기까지의 문제·쐐기·10-star가 **모두 같은 전제(프레이밍) 위에 서 있지 않은가?** 그렇다면 한 골짜기 안에서만 최적화한 것(국소 최적)이다. 산출물을 내기 전에 한 번 흔든다.

1. 이 문제를 **반대로** 정의하면? (해결하는 대신 제거·회피·무의미화)
2. 핵심 제약을 **없애면**? 반대로 **극단**으로 밀면?
3. 지금 사용자가 아니라 **전혀 다른 사용자·맥락**의 문제라면?
4. 5년 전·후라면 같은 답인가?

흔들어 더 나은 프레이밍이 나오면 위 단계로 **다시 돌아간다**(restart). 안 나오면 그대로 산출물로 간다. 이건 더 나은 골짜기를 **사람에게 제안**하는 단계지, AI가 혼자 점프를 확정하는 게 아니다(§0 — 방향은 사람). 큰·근본 결정에서만 깊게 돌리고, 작은 일엔 가볍게 한 번 훑는다(§15).
