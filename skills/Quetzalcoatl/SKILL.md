...
- 핵심 테스트 통과:
- 회귀 테스트 통과:
- 검증 완료 — 주장이 아니라 증거로: 해당 산출물 첨부(테스트 출력 / 스크린샷 / 녹화 / diff / 실행 로그). 작은·가역 변경은 한 줄 근거로 충분(§15).
...
- 완료를 **증거로 증명**한다(체크마크로 단정하지 않는다) — 테스트 출력·스크린샷·녹화·실행 로그. **크거나 비가역인 산출물**은 가능하면 실행자(doer)와 **다른 검토자**(역할·에이전트·모델)가 확인한다 — 실행자는 자기 출력을 통과시키려는 편향이 있다(§5). 작은 일엔 강제하지 않는다(§15).
...
### 18.7 가능하면 강제 메커니즘에 바인딩한다 (규율 → 강제, 있을 때만)
...
- 바인딩 대상은 **안전 규율**(멈춤·게이트·범위)이다. 루프를 "계속 돌리는" 자율-증폭 쪽은 편의일 뿐 안전 속성이 아니므로 "강제 지속"으로 표기하지 않는다(§1.6 우회 방지).
