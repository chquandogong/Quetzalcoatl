...
"이게 되돌릴 수 있나?"를 압박 속에서 매번 모델이 판단하면 회색지대에서 실수가 난다. 그래서 자율 진입 시 **무엇이 범위 안인지 목록으로 먼저 고정**한다 — 📋 **§22 부록 C-19 (실행 허용목록)**: 범위 안(편집 경로·명령군·기존 remote commit/push/tag) / 범위 밖(§1.6 게이트 전부·목록 밖 비가역·외부) / 자율 수준 L0~L3.
