...
```bash
# 개인 스킬 디렉터리로 클론
git clone <repo-url> ~/.claude/skills/Quetzalcoatl

# 또는 저장소를 따로 두고 심볼릭 링크로 연결 (단일 진실 소스)
git clone <repo-url> ~/repos/Quetzalcoatl
ln -s ~/repos/Quetzalcoatl ~/.claude/skills/Quetzalcoatl
```
...
전체 모드는 [`SKILL.md`](./SKILL.md) §2, 자세한 사용법은 [`USAGE.md`](./USAGE.md) 참고.
