...
- **실제 GPT-5.5 교차검증**(Codex exec, xhigh; ACCEPT-WITH-CHANGES 88) — Claude 자가 저작 §24를 GPT가 독립 비평(§5.2). **R13 중립 위반**(본문 도구명)·온보딩 필드 부족·과장·반출 위생 6건 적발 → **릴리스 전 §24 전면 개정 반영**. 단일모델 사각을 실제 교차검증이 두 번째로 적발([CROSS_VALIDATION_LOG](docs/02-decisions/CROSS_VALIDATION_LOG.md)).
