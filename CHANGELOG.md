...
**로컬 미니멈(프레이밍 잠김) 탈출.** 양방향 앵커링(한 *점*에 닻) 방지에 더해, 후보가 같은 전제를 공유한 채 한 골짜기 안에서만 고른 **국소 최적**을 벗어나는 기제를 정식화.

### Added

- **§3 Phase 1-7 "프레이밍을 흔든다".** Office Hours 산출물 직전 점프 스텝 — 무엇을 최적화하나(목표함수)·반대로·제약 없애기/극단·다른 사용자·시간축의 5개 질문. **흔들어 나온 대안 프레이밍은 말로 끝내지 말고 실제 후보로 §4.1 비교표에 넣게 강제**(질문만으론 정당화로 끝남). restart는 AI 단독이 아니라 **사람에게 제안→채택 시 복귀**(§0). 가드를 도입부로 전진(큰·비가역·프레이밍 의심 시만). 기존 Office Hours 1~6 번호 불변 → C-2·C-3 참조 무손상.
- **§4.1 골짜기 점검.** 되돌리기 어렵거나 비용 큰 결정에서만, 비교 전 "후보들이 같은 전제를 공유하나?" 점검 후 전제 깨는 후보를 최소 하나 더 후보표에 추가. 작고 가역적 결정엔 생략(§15).
- **§4.2 규칙 7 — 탐색 깊이·폭 다이얼.** 선호 유지 시 반론 강도를 사용자가 조절(기본=한 줄, "적극" 시 깊이↑). 깊이만으론 같은 전제 안 국소 최적에 갇히므로 **폭**(공유 전제 깨고 다른 프레이밍에서 후보 재생성)을 명시. 앵커=점, 국소 최적=골짜기. 작고 가역적 결정엔 미적용(§15).

### Verified

- **실제 GPT-5.5 교차검증**(Codex exec, 적대적 §5.4; **ACCEPT-WITH-CHANGES 78**) — diff 독립 비평. "질문만 더 하면 흔드는 척으로 기존 프레이밍을 정당화한다 → 대안 프레이밍을 **실제 후보로 같은 표에서 비교**해야 탈출" 등 5건 적발 → 릴리스 전 §3 Phase 1-7 개정 반영(목표함수 질문 추가·후보 생성 강제·restart 사람승인·가드 전진). 단일모델 사각을 실제 교차검증이 또 적발([CROSS_VALIDATION_LOG](docs/02-decisions/CROSS_VALIDATION_LOG.md)).

### Note

- PATCH — 동작 보강(새 모드 없음), v2.0.0 동작 호환. 양방향 앵커링(점) + 로컬 미니멈(골짜기) 탈출을 §3 Phase 1-7 + §4.1 + §4.2 세 곳으로. 미러(dashboard.html) 본문 redeploy는 후속(§7.2).
