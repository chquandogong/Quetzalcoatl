...
- **§4.2 규칙 7 — 탐색 깊이·폭 다이얼.** 선호 유지 시 반론 강도를 사용자가 조절(기본=한 줄, "적극" 시 깊이↑). 깊이만으론 후보가 공유 전제를 못 벗어나 **국소 최적(로컬 미니멈)**에 갇히므로, **폭** 다이얼(공유 전제 깨고 다른 프레이밍에서 후보 재생성)을 명시. 앵커=점, 국소 최적=골짜기. 기본은 §15 따라 가볍게 — 작고 가역적 결정엔 미적용.
- **§3 Phase 1-7 "프레이밍을 흔든다(골짜기 탈출)".** Office Hours 산출물 직전에 점프 스텝 추가 — 반대로·제약 없애기/극단·다른 사용자·시간축으로 문제 프레이밍을 흔들고, 더 나은 프레이밍이 나오면 restart. 점프 확정이 아니라 **사람에게 제안**(§0). annealing/restart 기제 정식화. 기존 Office Hours 1~6 번호 불변 → C-2·C-3 참조 무손상.
- **§4.1 골짜기 점검.** 되돌리기 어렵거나 비용 큰 결정에서만, 비교 전 "후보들이 같은 전제를 공유하나?" 점검 후 전제 깨는 후보를 하나 더 추가. 작고 가역적 결정엔 생략(§15). §3 Phase 1-7과 짝.

### Note

- PATCH 예정(동작 보강, 새 모드 없음). 본문만 변경 — **릴리스(3중 버전업·tag·Release·재설치) 보류, 로컬 커밋**(1.6.0 릴리스 트레드밀 교훈). 양방향 앵커링(점) 방지에 더해 **로컬 미니멈(골짜기) 탈출**을 §3 Phase 1-7 점프 + §4.1 골짜기 점검 + §4.2 폭 다이얼 세 곳으로 정식화.
